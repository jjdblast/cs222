...
# Task 1: Compute the moon's position (a vector) at time t. Let it start at moon_initial_angle, not on the horizontal axis.   
    ###Your code here.
    moon_angle = moon_initial_angle + 2.0*math.pi/moon_period*time
...
# Task 2: Compute the spacecraft's acceleration due to gravity
    ###Your code here.
    vectorToEarth = -position
    accEarth = gravitational_constant*earth_mass*vectorToEarth/numpy.linalg.norm(vectorToEarth)**3
    vectorToMoon = moon_position(time) - position
    accMoon = gravitational_constant*moon_mass*vectorToMoon/numpy.linalg.norm(vectorToMoon)**3
    acc = accEarth + accMoon
    return acc  
...
def apply_boost():
    ...
    while current_time < total_duration:
        ...
        velocityE=velocity+h*acceleration(current_time,position)
        positionH=position+h*0.5*(velocity+velocityE)
        velocityH=velocity+h*0.5*(acceleration(current_time,position)+acceleration(current_time+h,positionE))
        position=positionH
        velocity=velocityH

        ###Your code here.
        LTE=numpy.linalg.norm(positionE-positionH)+total_duration*numpy.linalg.norm(velocityE-velocityH)
